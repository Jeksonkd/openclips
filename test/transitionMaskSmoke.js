// Runs the same solid red -> green setup through each new transition mask
// type (wipe/iris/dissolve) so their per-pixel geq masks can be verified
// against known-good pixel expectations (e.g. wipe should be a hard 0/1 edge
// sweeping left-to-right, not a smooth fade).
...
const type = process.argv[2] || 'wipe';
const TDUR = 1.0;
// Matches ProjectState.applyTransition's actual behavior: mask transitions
// (wipe/iris/dissolve) only put the mask on the incoming clip - the outgoing
// clip stays fully opaque and just gets covered, since two independent hard
// masks would leave gaps where neither clip is visible.
const clipR = clip({ mediaId: 'mR', startTime: 0, inPoint: 0, outPoint: 4, fadeOut: TDUR });
const clipG = clip({
  mediaId: 'mG', startTime: 4 - TDUR, inPoint: 0, outPoint: 4,
  animIn: { type, duration: TDUR }, fadeIn: TDUR,
  transitionIn: { type, duration: TDUR, withClipId: clipR.id },
});
...
const outPath = `/tmp/cc-test/transition_${type}_smoke.mp4`;
